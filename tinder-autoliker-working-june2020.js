...
function trickTinder(runnumber) {
  ...
  if(runnumber=="firstrun") {
    var like = document.getElementsByClassName("button Lts($ls-s) Z(0) CenterAlign Mx(a) Cur(p) Tt(u) Bdrs(50%) P(0) Fw($semibold) focus-button-style Bxsh($bxsh-btn) Expand D(b) Bgc(#fff) Trstf(e) Trsdu($normal) Wc($transform) Pe(a) Scale(1.1):h Scale(.9):a")[1];
  }
...
(function loopSasori(runnumber="firstrun") {
  ...
  setTimeout(function() {
    ...
    if (checkTinder()) {
      var delay = trickTinder(runnumber);
      if (delay) {
